...
// src/event/functions/validate-guest-fields.ts
function validateGuestFields(guest) {
  const errs = [];
  if (!guest.name) errs.push("Nome \xE9 obrigat\xF3rio");
  if (!guest.email) errs.push("E-mail \xE9 obrigat\xF3rio");
  return errs;
}

// src/event/functions/complement-guest.ts
function complementGuest(partial) {
  const errs = validateGuestFields(partial);
  if (errs.length > 0) throw new Error(errs.join("\n"));
  const amountOfCompanions = partial.amountOfCompanions ?? 0;
  const hasACompanions = partial.confirmed && partial.hasACompanions && amountOfCompanions > 0;
  const guest = {
    ...partial,
...
// src/event/functions/validate-event-fields.ts
function validateEventFields(event) {
  const errs = [];
  if (!event.name) errs.push("Nome \xE9 obrigat\xF3rio");
  if (!event.location) errs.push("Local \xE9 obrigat\xF3rio");
  if (!event.date) errs.push("Data \xE9 obrigat\xF3rio");
  if (!event.expectedAudience || event.expectedAudience < 1)
    errs.push("P\xFAblico esperado \xE9 obrigat\xF3rio");
  return errs;
}
...
((DateUtils2) => {
  ...
  DateUtils2.parse = parse;
})(DateUtils || (DateUtils = {}));

// src/event/functions/complement-event.ts
function complementEvent(partial) {
...
// src/event/functions/create-empty-event.ts
...
// src/event/functions/create-empty-guest.ts
